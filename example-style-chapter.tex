...
\begin{framed}
  ...
  of this template is its more \myquote{traditional} \LaTeX{}
  implementation. But the contained information regarding \myquote{How
...
\item Do \emph{not} use slang abbreviations such as \myquote{it's},
  \myquote{doesn't}, or \myquote{don't}.

Write the words out in full: \myquote{it is}, \myquote{does not}, and \myquote{do not}.
...
\item Do \emph{not} use abbreviations such as \myquote{e.\,g.} or
  \myquote{i.\,e.}. 

Write the words out in full: \myquote{for example} and \myquote{that is}.

\mybadgood{\ldots in a tree, e.\,g.\xspace{}the items\ldots}{\ldots in a tree, for example the items\ldots}



\item Do \emph{not} use slang such as \myquote{a lot of}.

\mybadgood{There are a lot of features\ldots}{There are many features\ldots}



\item Do \emph{not} use slang such as \myquote{OK} or \myquote{big}.

\mybadgood{\ldots are represented by big areas.}{\ldots are represented by large areas.}



\item Do \emph{not} use slang such as \myquote{gets} or \myquote{got}.

Use \myquote{becomes} or \myquote{obtains}, or use the passive voice (third
person).
...
\item \emph{Never} start a sentence with \myquote{But}.

Use \myquote{However,} or \myquote{Nevertheless,}. Or consider joining the
...
\item \emph{Never} start a sentence with \myquote{Because}.

Use \myquote{Since}, \myquote{Owing to}, or \myquote{Due to}. Or turn the two
halves of the sentence around.




\item \emph{Never} start a sentence with \myquote{Also}. Also should
...
\item Do \emph{not} use \myquote{that} as a connecting word.
...
If you mean \myquote{aktuell} in German, you probably mean
\myquote{current} in English.
...
\item \myquote{allows to} is not English.
...
\item \myquote{As opposed to} is not English.

Use \myquote{In contrast to}.

\mybadgood{As opposed to C, Java is object-oriented.}{In contrast to C, Java is object-oriented.}


\item \myquote{\emph{anything}-dimensional} is spelt with a hyphen.

For example: two-dimensional, three-dimensional.



\item \myquote{\emph{anything}-based} is spelt with a hyphen.

For example: tree-based, location-based.



\item \myquote{\emph{anything}-oriented} is spelt with a hyphen.

For example: object-oriented, display-oriented.


\item \myquote{\emph{anything}-side} is spelt with a hyphen.

For example: client-side, server-side.


\item \myquote{\emph{anything}-friendly} is spelt with a hyphen.

For example: user-friendly, customer-friendly.


\item \myquote{\emph{anything}-to-use} is spelt with hyphens.

For example: hard-to-use, easy-to-use.



\item \myquote{realtime} is spelt with a hyphen if used as
...
\item Use common and basic vocabulary. For example, use \myquote{unusual}
  for \myquote{arcane}, and \myquote{specialised} for \myquote{erudite}.
...
\item Avoid \myquote{insider} comments. Ensure that your whole audience
...
\item Do not \myquote{play on words}. For example, do not use \myquote{puns},
...
\item If you are speaking of the holder of a position and you know the
  ...
  appropriate gender pronoun.  For example, suppose the \myquote{head nurse}
  is a man:
...
\item Avoid writing awkward formulations such as \myquote{s/he}, \myquote{he/she},
  or \myquote{his/her}.  They interfere when someone is trying to read a
  text aloud.  If none of the other guidelines has been helpful, use
  the slightly less awkward forms \myquote{he or she}, and \myquote{his or hers}.
